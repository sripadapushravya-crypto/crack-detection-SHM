...
def run_inference(
    ...
    result["predicted_label"] = [labels[int(value)] for value in predicted]
    result["crack_probability"] = probs
    result["confidence"] = [prob if pred == 1 else 1.0 - prob for pred, prob in zip(predicted, probs)]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run crack classifier inference across manifest images.")
    ...
    parser.add_argument("--limit", type=int, default=0, help="0 means process every manifest image.")
